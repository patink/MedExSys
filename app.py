...
def main():
	...
	if choice == "Home":
		...
		st.image("hepimage.jpg")
		st.text("Hepatitis B is a viral infection that attacks the liver and can cause both acute and \nchronic disease. Hepatitis B is spread when blood, semen, or other body fluids from a \nperson infected with the virus enters the body of someone who is not infected. Symptoms \nare variable and include yellowing of the eyes, abdominal pain and dark urine. Some \npeople, particularly children, don't experience any symptoms. In chronic cases, liver \nfailure, cancer or scarring can occur. The condition often clears up on its own. Chronic \ncases require medication and possibly a liver transplant.")
		...
		if st.sidebar.checkbox("Login"):
			if password == "12345":
				...
				if activity == "Plot":
					...
					f = grouped.get_group(2)
					b=st.write(sns.heatmap(f,annot=True))
					st.pyplot()
